...
from lex import tokens, newLexer
import os
...
import pandas as pd
import re
import GalaktionCore as G
from GalaktionCore import ExpressionType as ExprT, Expression, Symbol, SymbolType as SymT, Checks, ActivationRecord, ActivationRecordManager
...
def p_generatestmt(p):
    '''
    generatestmt : GENERATE decl AS STRING_VALUE drctvs
    '''
    if p[2] == afterErrorExpression:
        ...
        if not p[2].symbol.name.startswith(symbolTable._temporarySymbolPrefix):
            if Checks.isExtensionValid(t, p[4]):
                p[2].symbol.value = p[4]
...
def p_extractstmt(p):
    ...
    if p[4] != None:
        ...
        if '@latlong' in p[4]:
            ...
            # TODO directive for file name extraction
            pass
...
def p_update_files(p):
    ...
    if symbolTable.get('GALAKTION_ROOT').value == None:
        p[0] = afterErrorExpression
        UI.emitError(G.Error(
            'GALAKTION_ROOT must be defined at least once prior to any get or extract instruction'))
    elif linkCode == None:
        p[0] = afterErrorExpression
        ...
        for f in activRecs.top().linkingFiles:
            for element in f.elements:
                if element == None:
                    UI.emitWarning(G.Warning(f.path + ': Tag was not found'))
                else:
                    ...
                    if path == None:
                        UI.emitWarning(
                            G.Warning(f.path + '.' + element.tag + ': Failed to link', p.lineno(0)))
                    else:
    ...
    if len(p) == 2:
        ...
        for i in range(len(activRecs.top().linkingFiles)):
            ...
            for element in activRecs.top().linkingFiles[i].elements:
                if element == None:
                    UI.emitWarning(
                        G.Warning(activRecs.top().linkingFiles[i].path + ': Tag was not found'))
                else:
                    ...
                    if type(p[4]) == G.Index:
                        ...
                        newl.append(l[0])
                    else:  # TODO is it necesary??
                        p[0] = afterErrorExpression
                        UI.emitError(
                            G.Error('Invalide slice or index', p.lineno(3)))
...
def p_tag(p):
    ...
    for i in range(len(activRecs.top().linkingFiles)):
        ...
        if type(p[3]) == G.Index:
            ...
            UI.emitError(G.Error('Invalide slice or index', p.lineno(2)))
            p[0] = afterErrorExpression
...
symbolTable.insert(Symbol(SymT.folder, False, 'GALAKTION_ROOT', None))
